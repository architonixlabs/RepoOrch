...
2. Update the matching fields in `.repo-orchestrator/registry.json` — including all contract fields (`authContracts`, `errorContracts`, `configContracts`, `dataContracts`, `serviceLevel`, `testContracts`) if present.
3. Set `userEdited: true` on this registry entry.
4. Validate the registry against `schemas/registry.schema.json`.
5. Save the registry.

Report: "Context for `<repo>` ingested into registry. Run `/repo-orch-sync <repo>` to also refresh the agent file if owns/endpoints changed."
